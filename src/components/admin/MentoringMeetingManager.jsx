...
// embedded=true: renders content directly without Card wrapper (for use inside details/accordion)
export default function MentoringMeetingManager({ interns = [], experts = [], preselectedInternId = null, embedded = false }) {
  const [open, setOpen] = useState(false);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({
    title: '', date: '', time: '', location: '',
    intern_ids: preselectedInternId ? [preselectedInternId] : [],
    expert_ids: [], notes: ''
  });
  const queryClient = useQueryClient();

  const { data: meetings = [] } = useQuery({
    queryKey: ['mentoring-meetings', preselectedInternId],
    queryFn: () => preselectedInternId
      ? base44.entities.MentoringMeeting.list('-meeting_date').then(all => all.filter(m => m.intern_ids?.includes(preselectedInternId)))
      : base44.entities.MentoringMeeting.list('-meeting_date'),
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ title: '', date: '', time: '', location: '', intern_ids: preselectedInternId ? [preselectedInternId] : [], expert_ids: [], notes: '' });
    }
  });
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 space-y-3">
          ...
          <Input placeholder="נושא / כותרת הפגישה *" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
          ...
          <div>
            <label className="text-xs text-slate-500 mb-1 block">מיקום</label>
            <Input placeholder="חדר ישיבות, קליניקה..." value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} />
          </div>
          <div>
            <label className="text-xs text-slate-600 font-medium mb-2 block">מתמחים מוזמנים</label>
          ...
          <div className="flex gap-2 pt-1">
            <Button onClick={handleSubmit} disabled={!form.title || !form.date || createMutation.isPending} className="bg-indigo-600 hover:bg-indigo-700 text-white">
  ...
  return (
    <Card className="border-0 shadow-lg mb-8">
      ...
      {open && <CardContent className="pt-0">{innerContent}</CardContent>}
    </Card>
  );
}